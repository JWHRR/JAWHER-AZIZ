...
export default function Restaurant() {
  ...
  const getTaskStatus = (r: RepasType, hasLog: boolean) => {
    ...
    let label = "En cours";
    if (r === "PETIT_DEJEUNER") label = "07:00 - 08:00";
    else if (r === "DEJEUNER") label = "12:00 - 13:00";
    else if (r === "DINER") label = "19:00 - 20:00";
    
    return { label, color: "bg-yellow-500 hover:bg-yellow-600 text-white" };
  };
                  ...
                  <div key={r} className="p-4 rounded-lg border bg-card">
                    ...
                    {isAdmin && (
                      <div className="text-sm text-muted-foreground mb-3">
                        <span className="font-medium text-foreground">Affecté à: </span>
                    ...
                    {isUserAssigned && !isAdmin && (
                      <Button size="sm" variant={myLog ? "outline" : "default"} className="w-full mt-2" onClick={() => openLog(r)}>
                        {myLog ? <><Eye className="h-3.5 w-3.5 mr-1" /> Modifier</> : <><Plus className="h-3.5 w-3.5 mr-1" /> Suivi & validation</>}
                      </Button>
                    )}
                    {!isUserAssigned && !isAdmin && (
